...
for j in range(6):
    dataframe = pd.read_excel(r"D:\Users\s9870443\Github\Lastgangsynthese\Haushalte_V4a.xls", sheet_name=f"HH-Typ{j+1}")
    ...
    process_profiles_dc = [
        {
            ...
            "active_power_2_distribution_type": process_profiles.iloc[i * 4 + 2, [6]].fillna("normal").to_numpy()[0],
            ...
            "reactive_power_2_distribution_type": process_profiles.iloc[i * 4 + 2, [12]].fillna("normal").to_numpy()[0],
    ...
    lighting_profiles_dc = [
        {
            ...
            "reactive_power_distribution_type": lighting_profiles.iloc[i * 9 + 1, [6]].fillna("normal").to_numpy()[0],
    ...
    hhs = Households.from_config(hh)
    hh_c = converter.unstructure(hhs)
    path = pathlib.Path(f"data/household/dickert/hh{j+1}.json")
